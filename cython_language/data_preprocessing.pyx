...
class OutlierRemover(BaseEstimator, TransformerMixin):
    def __init__(self, list feat_with_outliers=["Family member count", "Income", "Employment length"]):
    ...
    def transform(self, df: pd.DataFrame):
        if set(self.feat_with_outliers).issubset(df.columns):
            Q1 = df[self.feat_with_outliers].quantile(0.25).to_numpy()
            Q3 = df[self.feat_with_outliers].quantile(0.75).to_numpy()
            IQR = Q3 - Q1
...
class DropFeatures(BaseEstimator, TransformerMixin):
    def __init__(self, list feature_to_drop=["Has a mobile phone", "Children count", "Job title", "Account age"]):
...
class TimeConversionHandler(BaseEstimator, TransformerMixin):
    def __init__(self, list feat_with_days=["Employment length", "Age"]):
...
class RetireeHandler(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df: pd.DataFrame):
        cdef str column_name = "Employment length"
        
        if column_name in df.columns:
            df[column_name] = np.where(df[column_name] == 365243, 0, df[column_name])
            return df
        else:
...
class SkewnessHandler(BaseEstimator, TransformerMixin):
    def __init__(self, list feat_with_skewness=["Income", "Age"]):
    ...
    def transform(self, df: pd.DataFrame):
        if set(self.feat_with_skewness).issubset(df.columns):
            df[self.feat_with_skewness] = np.cbrt(df[self.feat_with_skewness].values)
            return df
        else:
            print("One or more features are not in the dataframe")
            return df
...
class BinningNumToYN(BaseEstimator, TransformerMixin):
    def __init__(self, list feat_with_num_enc=["Has a work phone", "Has a phone", "Has an email"]):
    ...
    def transform(self, df: pd.DataFrame):
        if set(self.feat_with_num_enc).issubset(df.columns):
            df[self.feat_with_num_enc] = df[self.feat_with_num_enc].replace({1: "Y", 0: "N"})
            return df
        else:
            print("One or more features are not in the dataframe")
            return df
...
class OneHotWithFeatNames(BaseEstimator, TransformerMixin):
    def __init__(self, list one_hot_enc_ft=[
                "Gender",
                "Marital status",
                "Dwelling",
                "Employment status",
        ...
        if set(self.one_hot_enc_ft).issubset(df.columns):
            ...
            def one_hot_enc(df, one_hot_enc_ft):
                ...
                df_encoded = pd.DataFrame(
                    one_hot_enc.transform(df[one_hot_enc_ft]).toarray(),
...
class OrdinalFeatNames(BaseEstimator, TransformerMixin):
    def __init__(self, list ordinal_enc_ft=["Education level"]):
    ...
    def transform(self, df: pd.DataFrame):
        cdef str column_name ="Education level"
        if column_name in df.columns:
...
class ChangeToNumTarget(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df: pd.DataFrame):
        cdef str column_name = "Is high risk"
        if column_name in df.columns:
